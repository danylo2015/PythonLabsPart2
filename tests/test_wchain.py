...
class TestWchain(unittest.TestCase):
    def test_default_condition(self):
        ...
        with open("E:\\PythonLabsPart2\\tests\\resources\\wchain_out.txt", "r", encoding="utf-8") as wchain_out:
            result = int(wchain_out.readline())
        self.assertEqual(result, 6)

    def test_empty_wchain(self):
        write_output_file("empty_wchain_in.txt", "empty_wchain_out.txt")
        with open("E:\\PythonLabsPart2\\tests\\resources\\empty_wchain_out.txt", "r", encoding="utf-8") as wchain_out:
            result = int(wchain_out.readline())
